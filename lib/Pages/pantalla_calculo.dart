...
class PantallaCalculo extends StatelessWidget {
  ...
  Future<void> guardarVentas() async {
    ...
    final ventasPrevias = prefs.getStringList('registro_diario') ?? [];

    for (var venta in ventas) {
      final ventaConPrecio = {
        'producto': venta['producto'],
        'cantidad': venta['cantidad'],
        'precio': obtenerPrecio(venta['producto']),
      };
      ventasPrevias.add(jsonEncode(ventaConPrecio));
    }

    await prefs.setStringList('registro_diario', ventasPrevias);
  }
          ...
          body: Padding(
            ...
            child: Column(
              children: [
                ...
                ElevatedButton.icon(
                  onPressed: () async {
                    await guardarVentas(); // ✅ Guarda ventas
                    onFinalizarVenta(ventas); // Limpia en pantalla anterior
